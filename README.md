...
- **Пунктуация и заглавные буквы** — восстанавливаются локальной LLM (Whisper теряет их на русском).
- **Локальное резюме** — тема, решения, действия, открытые вопросы через Ollama.
- **Автовставка своей речи** — как только транскрипция готова, ваш текст вставляется в окно с текстовым курсором (SuperWhisper-style), без дополнительных нажатий.
- **Папка на встречу** — `audio.wav`, `transcript.md`, `transcript.json`, `summary.md`, `meta.json`. Папка и есть база данных, как в оригинале.
