...
## Установка (установщик)

Скачайте `amanu-setup.exe` со страницы релизов и запустите. Мастер предложит
папку установки (без прав администратора, `%LOCALAPPDATA%\Programs\Amanu`),
три ступени качества распознавания (Точная ~1.6 ГБ / Сбалансированная ~460 МБ /
Компактная ~460 МБ), опциональные сводки через Ollama (~5 ГБ), ярлык на рабочий
стол и автозапуск. Сами модели скачиваются при первом запуске: окно прогресса
показывает шаги, умеет «Повторить» при обрыве сети и «Сделать это позже».
Деинсталлятор спрашивает, удалять ли модели и записи — по умолчанию они
остаются.

Собрать установщик самому: `winget install JRSoftware.InnoSetup`, затем
`python scripts/build_installer.py` — рядом с бандлом появится `amanu-setup.exe`.
...
**Выбор микрофона — автоматический (shotgun-старт):** при старте записи первые ~2.5 с пишутся **все** активные входы, архив получает тот, где есть живой сигнал (мёртвые виртуальные драйверы шлют синтетические нули и проигрывают автоматически). Первые слова не теряются — победитель писал с t=0. Жёсткий пин: `"mic_device": "SonoFlo"` в конфиге отключает shotgun и открывает ровно это устройство.
